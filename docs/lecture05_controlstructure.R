...
require(ggplot2)
df <- data.frame(vitamine = rep(c("OJ","VC"),each=B), bootstrap = c(boot_oj, boot_vc))
p <- ggplot(df, aes(x=bootstrap)) + 
  geom_histogram(data = subset(df, vitamine == "OJ"), fill = "red", alpha = 0.5) +
  geom_histogram(data = subset(df, vitamine == "VC"), fill = "green", alpha = 0.5)
p
